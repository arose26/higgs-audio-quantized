    ...
    if ref_audio is not None:
            ...
            for spk_id, character_name in enumerate(speaker_info_l):
                if character_name.startswith("profile:"):
                    if voice_profile is None:
                        with open(f"{CURR_DIR}/examples/voice_prompts/profile.yaml", "r", encoding="utf-8") as f:
        ...
        for spk_id, character_name in enumerate(ref_audio.split(",")):
            if not character_name.startswith("profile:"):
                prompt_audio_path = os.path.join(f"{CURR_DIR}/examples/voice_prompts", f"{character_name}.wav")
                prompt_text_path = os.path.join(f"{CURR_DIR}/examples/voice_prompts", f"{character_name}.txt")
...
def load_model_and_tokenizer(
    ...
    """Load the model and audio tokenizer.
    
    Parameters
    ----------
    model_path : str
        Path to the HiggsAudio model
    audio_tokenizer : str
        Path to the audio tokenizer
    max_new_tokens : int
        Maximum number of new tokens to generate
    device_id : int, optional
        CUDA device ID to use. If None, will auto-select
    use_static_kv_cache : int
        Whether to use static KV cache
...
def main(
    ...
    """Generate audio from text using a pre-loaded model.
    
    Parameters
    ----------
    model_client : HiggsAudioModelClient
        Pre-loaded model client for audio generation
    transcript : str
        Path to transcript file or text to convert to audio
    scene_prompt : str
        Path to scene prompt file or scene description
    temperature : float
    ...
    ras_win_len : int
        RAS window length parameter
    ras_win_max_num_repeat : int
        RAS max number of repeats parameter
    ref_audio : str, optional
        Reference audio for voice cloning
    ref_audio_in_system_message : bool
        Whether to include reference audio in system message
    chunk_method : str, optional
        Method for chunking text ("speaker", "word", or None)
    chunk_max_word_num : int
        Maximum words per chunk for "word" chunking
    chunk_max_num_turns : int
        Maximum turns per chunk for "speaker" chunking
    generation_chunk_buffer_size : int, optional
        Buffer size for generation chunks
    seed : int, optional
        Random seed for generation
    out_path : str
        Output path for generated audio file
    """
...
def run_generation(
    ...
    """Convenience function that loads the model and runs generation.
    
    This function provides backward compatibility with the original main() interface.
    For more control, use load_model_and_tokenizer() followed by main() directly.
    ...
    use_static_kv_cache : int
        Whether to use static KV cache
    **kwargs
        All other parameters are passed to main()
    """
